...
class Agent():
    def __init__(self, env, epsilon=0.95, learning_rate=0.05, GAMMA=0.97, num_bins=7):
        """
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        # Update Q table
        maxFutureQ = np.max(self.qtable[tuple(next_state)])  
        currentQ = self.qtable[tuple(state) + (action, )]  

        newQ = (1 - self.learning_rate) * currentQ + self.learning_rate * (reward + self.gamma * maxFutureQ)
...
def train(env):
    ...
    for ep in tqdm(range(episode)):
        ...
        training_agent.epsilon = max(0.2, training_agent.epsilon)
        ...
        if ep >= 20:
          if ep%200 == 0:
            ...
            print(f"epsilon: {training_agent.epsilon}")
            print(f"learning rate: {training_agent.learning_rate}")
